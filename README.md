...
### allow は配置要求として他プラグインへ宣言される

`allow` は「ハードコード値を許可する定数ファイルをここに置く」という要求であるため、配置を検査するプラグイン（`ai-harness-directory-checker` 等）へ**配置要求として宣言**される。

宣言先がその配置を許可していなければ、定数ファイルを作った途端に弾かれる＝設定が両立していない。この矛盾は**起動時にエラー**となり、そのプロジェクトの hook は全てブロックされる（フェイルクローズ）。`ai-harness-main --validate` で hook を待たずに確認できる。

- 宣言によって相手側の許可が広がることはない。矛盾を露見させるためだけの一方向の通知
- 宣言先プラグインが無効・未導入なら宣言は誰にも読まれず、何も起きない
- 雛形（有効化時に自動生成される既定設定）のプレースホルダは宣言しない。設定を実プロジェクトのパスへ書き換えるまで、この検証は働かない
